...
export default function splash({
  ...
  let div = dom(
    '.splash',
    !iframe && dom('.logos', logo && dom('.logo.org'), dom('.logo.slack')),
    dom('p', dom('b', name)),
    dom('p', 'We invite you to join us.'),
    dom(
      'p.status',
      active
        ? [
            dom('b.active', active),
            ' members online now of ',
            dom('b.total', total),
            ' total.'
          ]
        : [dom('b.total', total), ' humans have joined so far.']
    ),
    dom(
      'form id=invite',
      channels &&
      ...
      dom('button.loading', 'Join Community')
    ),
    dom(
      'p.signin',
      'or ',
    ...
    !iframe &&
      dom(
        ...
        'a piece of ',
        dom(
          'a href=https://intentional.earth target=_blank',
          'intentional.earth'
        )
      ),
